...
out_dir<-'//catalogue/BaseLineDataCluster01/temp/dssat_outputs/ethiopia/tpe/PlantGro/final/cluster/data_plot_stress/corridor_cluster_prob/space_time_ethi/'
cntry<-'ethiopia'
...
for (n in 1:10){
  ...
  hist<-transform(hist,cluster=factor(cluster,levels=c('SF','GS','MTS','ETS')))
  fut<-transform(fut,cluster=factor(cluster,levels=c('SF','GS','MTS','ETS')))
  ...
  p_hist<-ggplot(hist, aes(x=period, y = Probability, fill = cluster, label =Probability)) +
    ...
    xlab('') + ylab('Proportion') + scale_fill_manual(values=c('#377eb8','#ec7014','#e41a1c','#7f2704')) +
  ...
  P_fut<-ggplot(fut, aes(x = ssp, y = Probability, fill = cluster, label =Probability)) +
    ...
    scale_y_continuous(breaks = seq(0.2,1,0.2)) +
    xlab('SSP') + ylab('') + scale_fill_manual(values=c('#377eb8', '#ec7014','#e41a1c','#7f2704')) +
    geom_text(size = 3, position = position_stack(vjust = 0.5)) + labs(fill='') +
