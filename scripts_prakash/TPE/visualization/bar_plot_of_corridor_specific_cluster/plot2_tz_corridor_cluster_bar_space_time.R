...
for (n in 1:length(region)){
  ...
  ## fill here corrections
    My_Theme = theme(legend.position='bottom', legend.text = element_text(color = "black", size = 6),
  ...
  hist<-transform(hist,cluster=factor(cluster,levels=c('SF','AS','MTS','STS', 'ETS')))
  p_hist<-ggplot(hist, aes(x=period, y = Probability, fill = cluster, label =Probability)) +
    geom_bar(stat = "identity") + ggtitle("Historical") +
    xlab('') + ylab('Proportion') + scale_fill_manual(values=c('#377eb8','#feb24c','#e31a1c','#67000d','#984ea3')) +
    geom_text(size = 3, position = position_stack(vjust = 0.5)) + labs(fill='') +
    theme_bw() + theme(legend.position='bottom', plot.title = element_text(size=9)) + My_Theme 
  
  fut<-transform(fut,cluster=factor(cluster,levels=c('SF','AS','MTS','STS', 'ETS')))
  P_fut<-ggplot(fut, aes(x = ssp, y = Probability, fill = cluster, label =Probability)) +
    geom_bar(stat = "identity") + facet_grid(.~period) +
    xlab('SSP') + ylab('') + scale_fill_manual(values=c('#377eb8','#feb24c','#e31a1c','#67000d','#984ea3')) +
    geom_text(size = 3, position = position_stack(vjust = 0.5)) + labs(fill='') +
  ...
  ggsave(plot = plot, 
         filename = paste0(out_dir,region[n],'_',comb$year[n],'.png'), 
