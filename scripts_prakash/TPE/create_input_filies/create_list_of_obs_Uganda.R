...
dd<-readRDS('//dapadfs/workspace_cluster_12/AVISA/data_for_dssat_eaf/cc_bc/Uganda/obs/pr_tmx_tmin_rad_Ugan_1991_2010.RDS')
file<-nc_open('//dapadfs/workspace_cluster_12/AVISA/data_for_dssat_eaf/observed_baseline/chirps/nc/proc/chirps-v2.0.1991_2010.days_p05.ugan.nc')

obs_lt<-ncvar_get(file, 'latitude')
obs_ln<-ncvar_get(file, 'longitude') 
ln<-obs_ln[4:194]; lt<-obs_lt[3:218]
...
ln1<-which(as.character(ln)==29.525);    ln2<-which(as.character(ln)==35.025)
lt1<-which(as.character(lt)==-1.525);     lt2<-which(as.character(lt)==4.475) 
...
saveRDS(yyy,'//dapadfs/workspace_cluster_12/AVISA/data_for_dssat_eaf/dssat_input/Uganda/obs/agMeChrips_ugan_1991_2010.RDS')
q(save='no')
