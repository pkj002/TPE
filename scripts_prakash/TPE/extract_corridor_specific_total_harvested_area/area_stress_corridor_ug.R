...
extent<- c(xmn=29.125, xmx=35.025, ymn=-1.525, ymx=4.475)
...
for (n in 1:10){
  ...
  fut_f<-array(fut$Proportion,c(2,8))
  colnames(fut_f)<-c('ssp1_30','ssp2_30','ssp3_30','ssp4_30','ssp1_50','ssp2_50','ssp3_50','ssp4_50')
  all<-round(data.frame(History=hist$Proportion,fut_f)*hv_area[n],0)
  c_all[[n]]<-data.frame(Production_hub=region[n],clus=hist$cluster,all)
}

prop_all<-do.call(rbind,c_all)
prop_all<-data.frame(country='UGA',prop_all)
saveRDS(prop_all, '//catalogue/BaseLineDataCluster01/temp/area_stress_corridor_ug.RDS')