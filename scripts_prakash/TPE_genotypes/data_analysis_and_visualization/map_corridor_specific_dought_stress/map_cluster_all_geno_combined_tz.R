...
ext <- c(xmn=29.125, xmx=40.125, ymn=-11.975, ymx=-0.975)

## History 
cultivar <- c('Cal_96','DAB_489','HTA_4','KAT_B9','MCM_1015','SCR_26','SELIAN_97','SER_119','SUG_73','UYOLE_94')

yld1 <- list(); yld2 <- list(); yld3 <- list(); yld4 <- list(); yld5 <- list()

for (i in 1:10){
  # for (i in 6:10){
  ## cl1
  gg<-readRDS(paste0(path,'clust_perc_in_each_grid/clust1prop_c',i,'_tanzania_all_ge.RDS'))
  df1<-unlist(gg$clst)
  df1[is.na(df1)]=0
  
  ## cl2
  hh<-readRDS(paste0(path,'clust_perc_in_each_grid/clust2prop_c',i,'_tanzania_all_ge.RDS'))
  df2<-unlist(hh$clst)
  df2[is.na(df2)]=0
  
  ## cl3
  ii<-readRDS(paste0(path,'clust_perc_in_each_grid/clust3prop_c',i,'_tanzania_all_ge.RDS'))
  df3<-unlist(ii$clst)
  df3[is.na(df3)]=0
  
  ## cl4
  jj<-readRDS(paste0(path,'clust_perc_in_each_grid/clust4prop_c',i,'_tanzania_all_ge.RDS'))
  df4<-unlist(jj$clst)
  df4[is.na(df4)]=0
  
  ## cl5
  kk<-readRDS(paste0(path,'clust_perc_in_each_grid/clust5prop_c',i,'_tanzania_all_ge.RDS'))
  df5<-unlist(kk$clst)
  df5[is.na(df5)]=0
  
  cl2 <- df1; cl1 <- df2; cl5 <- df3; cl3 <- df4; cl4 <- df5
  yld1[[i]]<- array(cl1,c(ln,lt))*100
  yld2[[i]]<- array(cl2,c(ln,lt))*100  
  yld3[[i]]<- array(cl3,c(ln,lt))*100
  yld4[[i]]<- array(cl4,c(ln,lt))*100
  yld5[[i]]<- array(cl5,c(ln,lt))*100
  ...
  if(map=='1st'){
    yld<-abind(yld1[[1]],yld2[[1]],yld3[[1]],yld4[[1]],yld5[[1]],
               yld1[[2]],yld2[[2]],yld3[[2]],yld4[[2]],yld5[[2]],
               yld1[[3]],yld2[[3]],yld3[[3]],yld4[[3]],yld5[[3]],
               yld1[[4]],yld2[[4]],yld3[[4]],yld4[[4]],yld5[[4]],
               yld1[[7]],yld2[[7]],yld3[[7]],yld4[[7]],yld5[[7]],along=3)  
  } else {
    yld<-abind( 
      
      yld1[[9]],yld2[[9]],yld3[[9]],yld4[[9]],yld5[[9]],
      yld1[[10]],yld2[[10]],yld3[[10]],yld4[[10]],yld5[[10]],
      yld1[[6]],yld2[[6]],yld3[[6]],yld4[[6]],yld5[[6]],
      yld1[[8]],yld2[[8]],yld3[[8]],yld4[[8]],yld5[[8]], 
      yld1[[5]],yld2[[5]],yld3[[5]],yld4[[5]],yld5[[5]], 
      along=3)  
  }
  ...
  data<-brick(df2, xmn=29.125, xmx=40.125, ymn=-11.975, ymx=-0.975, crs=NA, transpose=TRUE)
  mps <- shapefile('C:/Users/pjha/Desktop/TZA_adm/TZA_adm0.shp')     
  
  # name for panel plots
  if(map=='1st'){
    name<-paste0(rep(cultivar[c(1:4,7)],each=5),c('_SF','_AS','_MTS','_STS','_ETS')) 
  } else {
    name<-paste0(rep(cultivar[c(9:10,6,8,5)],each=5),c('_SF','_AS','_MTS','_STS','_ETS')) 
  }
  
  crs(data)<-crs(mps)
  data1 <- crop(data, mps) %>% raster::mask(mps)
  names(data1)<-name
  ...
  if(map=='1st'){
    tmap_save(curmap,
              'E:/Prakash/dssat_out_culti/tanzania/maps/geno1to5_combined_cluster_freq.png', 
              width= 10, height = 10, units = "in",  dpi = 300)
  } else {
    tmap_save(curmap,
              'E:/Prakash/dssat_out_culti/tanzania/maps/geno6to10_combined_cluster_freq.png', 
              width= 10, height = 10, units = "in",  dpi = 300)
  }
