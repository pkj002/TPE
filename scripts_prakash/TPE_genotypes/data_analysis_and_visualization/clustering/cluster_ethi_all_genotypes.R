...
cntry<-'ethiopia'
path<-'E:/Prakash/dssat_out_culti/ethiopia/PlantGro/'

cul <- 1:10

stress <- list(); stress2 <- list()
for (n in 1:10){
  stress1 <- readRDS(paste0(path, 'PlantGro_c',cul[n],'_ethiopia.RDS'))
...
#colnames(gg) <- colnames(df) ## put name of the columns

saveRDS(gg,paste0(path,'cluster/cluster_ethiopia_all_genotypes_all_rows.RDS'))
