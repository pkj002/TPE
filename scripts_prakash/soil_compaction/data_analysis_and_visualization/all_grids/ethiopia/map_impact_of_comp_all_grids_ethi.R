...
## Load data
orig <- readRDS(paste0(in_path, 'data_orig_ethiopia.RDS'))
comp <- readRDS(paste0(in_path, 'data_compact_ethiopia.RDS'))

dif = round((comp[,2:15] - orig[,2:15])*100/orig[,2:15],1)
dif = data.frame(grids=orig$grids,dif)
...
cwam <- array(df1$cwam,c(ln,lt)); lai <- array(df1$lai,c(ln,lt)); et_c <- array(df1$et_c,c(ln,lt));
...
mps <- getData("GADM", country = "ETH", level = 1, path = in_path)
...
pcols <- c('#8c510a','#d8b365','#f6e8c3','#f5f5f5','#c7eae5','#5ab4ac','#01665e')
...
mps <- getData("GADM", country = "ETH", level = 0, path = in_path)
## draw plot
...
tmap_save(curmap,
          paste0(in_path,'ethi_impact_of_compaction.png'), 
