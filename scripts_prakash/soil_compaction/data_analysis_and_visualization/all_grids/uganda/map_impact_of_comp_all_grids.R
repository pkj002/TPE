...
## Load data
orig <- readRDS(paste0(in_path, 'data_orig_uganda.RDS'))
comp <- readRDS(paste0(in_path, 'data_compact_uganda.RDS'))
...
mps <- getData("GADM", country = "UGA", level = 1, path = in_path)
...
mps <- getData("GADM", country = "UGA", level = 0, path = in_path)
crs(atlas)<-crs(mps)
...
tmap_save(curmap,
          paste0(in_path,'ug_impact_of_compact.png'), 
