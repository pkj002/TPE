...
## Load data
orig <- readRDS(paste0(in_path, 'data_orig_',cntry,'.RDS'))
comp <- readRDS(paste0(in_path, 'data_compact_',cntry,'.RDS'))
...
curmap <- tm_shape(data1) +
  ...
  tm_layout(main.title='', main.title.size=1.5, panel.show = TRUE,   
            ...
            panel.labels = name, panel.label.size = 2.2, legend.outside.position = "bottom",
...
tmap_save(curmap,
          paste0(in_path,'tz_impact_of_compaction.png'), 
