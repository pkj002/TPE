runDSSATSingle_obs_cul <- function(n,dfile,outdir,indir){
...
if(nrow(x)>0){
    ...
    # copy files  
    file.copy(file.path(scripts, "original_files_tz_cultivars/BNGRO047.CUL"), out_dir)
    file.copy(file.path(scripts, "original_files_tz_cultivars/BNGRO047.SPE"), out_dir)
    file.copy(file.path(scripts, "original_files_tz_cultivars/BNGRO047.ECO"), out_dir)
    ...
    if (is.na(xs$jday_end_avg) == TRUE) {
      ...
      file.copy(file.path(scripts, "original_files_tz_cultivars/TANZ9105.BNX"), out_dir)
      file_x <- read_filex(file.path(out_dir,'TANZ9105.BNX'))
      
    } else {
      
      file.copy(file.path(scripts, "original_files_tz_cultivars/CCPA9105.BNX"), out_dir)
    ...
    if (is.na(xs$jday_end_avg) == TRUE) {
      ...
      CSMbatch_cul(crop="BEAN", name = "TANZ9105.BNX", xs, filename = file.path(out_dir,"DSSBatch.v47"))
      
    } else {
      write_filex(file_x, 'CCPA9105.BNX')
      CSMbatch_cul(crop="BEAN", name = "CCPA9105.BNX", xs, filename = file.path(out_dir,"DSSBatch.v47"))
    }
    ...
    # prepare output dir
      rds_dir <- file.path(outdir,paste0('obs_cul/', xs$Code))
    ...
    saveRDS(ev, paste0(rds_dir, '/Evaluate_tz_obs_cul_1to5.RDS'))  
