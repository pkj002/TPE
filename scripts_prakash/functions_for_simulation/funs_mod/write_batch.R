...
CSMbatch <- function(crop, name, x, filename) {
  ...
  outbatch <- rbind(rbind(rbind(rbind(rbind(rbind(rbind(rbind(rbind(rbind(
    rbind(
      # Batchfile headers            
      paste0("$BATCH(", crop, ")"),            
      "!",            
    ...
    if (cntry == 'ethiopia'){
	outbatch<-outbatch[1:8,]
	} else if(cntry == 'tanzania' && is.na(x$jday_end_avg) == TRUE){
	outbatch<-outbatch<-outbatch[1:7,]
	} else if(cntry == 'tanzania' && is.na(x$jday_end_avg) == FALSE){
	outbatch<-outbatch<-outbatch[1:11,]
	}
