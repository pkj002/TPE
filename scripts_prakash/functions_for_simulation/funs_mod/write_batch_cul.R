...
CSMbatch_cul <- function(crop, name, x, filename) {
...
cbind(sprintf("%6s %86s %6i %6i %6i %6i",paste0(name), 40, 1,0,1,0)))
 ...
 if(is.na(x$jday_end_avg) == TRUE){
  outbatch<-outbatch<-outbatch[1:23,]
} else {
  outbatch<-outbatch<-outbatch[1:43,]
}
