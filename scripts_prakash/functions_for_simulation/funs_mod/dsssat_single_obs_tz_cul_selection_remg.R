...
if(nrow(x)>0){
  ...
  #}
    ...
    if (is.na(xs$jday_end_avg) == TRUE) {
      ...
      file.copy(file.path(scripts, "original_files_tz_cultivars/remg_cul/TANZ9105.BNX"), out_dir)
      file_x <- read_filex(file.path(out_dir,'TANZ9105.BNX'))
      
    } else {
      
      file.copy(file.path(scripts, "original_files_tz_cultivars/remg_cul/CCPA9105.BNX"), out_dir)
    ...
    saveRDS(ev, paste0(rds_dir, '/Evaluate_tz_obs_cul_6to10.RDS'))

    pgr<-read.csv('plantgro.csv')
    pgr<-pgr[ ,c(1,3,6:ncol(pgr))]
    pgr<-round(pgr,3)	
	saveRDS(pgr, paste0(rds_dir, '/PlantGro_tz_obs_cul_6to10.RDS'))
