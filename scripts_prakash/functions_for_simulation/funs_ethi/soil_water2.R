soil_water2<-function(m, ff, sol, rho_b1){
  
  ## Read texture 
  silt0 = unlist(sol$SLSI)[1:2]
  clay0 = unlist(sol$SLCL)[1:2]
  sand0 = 100 - (silt0 + clay0)  
  
  ## Read SLOC, SSAT, SDUL, SLLL, SSKS and SBDM
  OC0 <- unlist(sol$SLOC)[1:2]; SSAT0 <- unlist(sol$SSAT)[1:2]; SDUL0<-unlist(sol$SDUL)[1:2] 
  SLLL0 <- unlist(sol$SLLL)[1:2]; SSKS0 <- unlist(sol$SSKS)[1:2]; rho_b0 = unlist(sol$SBDM)[1:2] 
