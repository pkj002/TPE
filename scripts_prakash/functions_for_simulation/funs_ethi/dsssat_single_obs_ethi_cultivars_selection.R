runDSSATSingle_obs_cul <- function(n,dfile,outdir,indir){
  ...
  #}
    ...
    file.copy(file.path(scripts, "original_files_ethi_cultivars/CCPA9105.BNX"), out_dir)
  ...
  # note
   ...
   ## make .RDS file from output files
    ...
    saveRDS(ev, paste0(rds_dir, '/Evaluate_ethiopia_obs_cul_1to5.RDS'))
	
	pgr<-read.csv('plantgro.csv')
   	 pgr<-pgr[ ,c(1,3,6:ncol(pgr))]
    pgr<-round(pgr,3)
	saveRDS(pgr, file.path(rds_dir, paste0('PlantGro_ethiopia_obs_cult_1to5.RDS')))
