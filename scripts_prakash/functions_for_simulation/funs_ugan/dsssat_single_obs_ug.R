runDSSATSingle_obs <- function(n,dfile,outdir,indir){
  ...
  if(nrow(xs)>0){
    ...
    out_dir <- file.path(outdir, paste0('run1/obs/', xs$Code)) 
  ...
  system(paste0("./dscsm047 B DSSBatch.v47"), ignore.stdout = TRUE)
    ...
    # prepare output dir
		rds_dir <- file.path(outdir,paste0('obs_new3/', xs$Code))
...
sum<-sum[,c(1,2,13:ncol(sum))]
sum <- rapply(object = sum, f = round, classes = "numeric", how = "replace", digits = 3) 
      
        
  lis <- list(ev,pgr,sum)
    
    ## make characater vector
    listnames <- c('Evaluate','PlantGro','Summary')
