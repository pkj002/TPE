runDSSATSingle_obs_bd_1lay <- function(n,dfile,outdir){
  ...
  if(nrow(xs)>0){
    ...
    if(ff>1){
      ...
      ## change BD
     if (ff== 2){
        #rho_b1 = rho_b0 + rho_b0*0.1 ## 10% increase
		rho_b1 = rho_b0 + rho_b0*0.17 ## 17% increase
      } else {
        rho_b1 = rho_b0
      }
      ...
      ## Calculate SDUL, SLLL, SSAT and SSKS for the increase in bulk density
       hhh<-sapply(1, soil_water1, ff, sol, rho_b1)
      
      ### Need to replace SDUL, SLLL, SSAT, SSKS with the modified values and
      sol$SSAT[[1]] <- round(c(unlist(hhh[1]),unlist(sol$SSAT)[2:6]),3)
      sol$SDUL[[1]] <- round(c(unlist(hhh[2]),unlist(sol$SDUL)[2:6]),3)
      sol$SLLL[[1]] <- round(c(unlist(hhh[3]),unlist(sol$SLLL)[2:6]),3)
      sol$SSKS[[1]] <- round(c(unlist(hhh[4]),unlist(sol$SSKS)[2:6]),2)
      
      #if(ff==3| ff==5){
        #sol$SRGF[[1]]<-sol$SRGF[[1]]*unlist(sol$SBDM)/round(c(rho_b1,unlist(sol$SBDM)[4:6]),2)
      #}
      
      sol$SBDM[[1]] <- round(c(rho_b1,unlist(sol$SBDM)[2:6]),2)
    ...
    file.copy(file.path(scripts, "original_files_ug/f17/CCPA9105.BNX"), out_dir)
    ...
    # prepare output dir
    rds_dir <- file.path(outdir,paste0('bd_random_lay1/', xs$Code,'/',soil[ff],'_',cultiv[gg]))
