runDSSATSingle_obs_wth <- function(n,dfile,outdir,indir){
  ...
  if(nrow(xs)>0){
    ...
    out_dir <- file.path(outdir, paste0('run/obs/', xs$Code)) 
  ...
  write_filex(file_x, 'CCPA9105.BNX')
   ...
   ## Run
  CSMbatch_17(crop="BEAN", name = "CCPA9105.BNX", xs, filename = file.path(out_dir,"DSSBatch.v47"))
  ...
  system(paste0("./dscsm047 B DSSBatch.v47"), ignore.stdout = TRUE)
    ...
    # prepare output dir
		rds_dir <- file.path(outdir,paste0('obs_weath_pltgr/', xs$Code))
